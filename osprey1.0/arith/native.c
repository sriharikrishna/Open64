...
/*
 * Solaris vendor compiler will evaluate false on all the following
...
#else	/* !(defined(_CRAYMPP) || defined(__mips)) */
...
/* 
 * Solaris porting
 * The following block contains a "undefined symbol" - opnd1type
 * it seems opnd1type should be basetype, since in this function (power)
 * there are only base/power these two operands.
 */
switch (AR_INT_SIZE (*basetype)) {
		case AR_INT_SIZE_8:
			AR_convert ((AR_DATA*) &tpow2, &inttype,
(AR_DATA*) &tpow,  basetype);
			break;
