...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = "$Source: /m_home/m_utkej/Argonne/cvs2svn/cvs/Open64/osprey1.0/be/whirl2f/w2f_driver.cxx,v $ $Revision: 1.1.1.1 $";
#endif
...
#define W2F_Extension(i) \
	(W2F_Prompf_Emission? W2F_Prompf_Extension[i] : \
...
Process_Filename_Options(const char *src_filename, const char *irb_filename)
{
   ...
   {
      if (List_Cite || W2F_Prompf_Emission)
      {
...
End_Locations_File(void)
{
   ...
   if (W2F_File_Name[W2F_LOC_FILE] != NULL)
   {
      ...
      {
	 Open_W2f_Output_File(W2F_LOC_FILE);
	 Write_String(W2F_File[W2F_LOC_FILE],
		      NULL/* No srcpos map */,
		      "SRCPOS_MAP_END\n");
      }
      else
...
{
   W2F_Prompf_Emission = TRUE;
   W2F_Construct_Map = construct_map; /* Construct id mapping for prompf */
...
// 	  pu = WN_Lower(pu, lower_actions, NULL, "W2F Lowering"); 
   ...
   BOOL nested = PU_is_nested_func(pucur);   
...
W2F_Outfile_Fini(void)
{
   ...
   if (W2F_Prompf_Emission && loc_fname != NULL)
   {
