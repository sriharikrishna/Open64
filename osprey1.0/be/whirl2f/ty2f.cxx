...
TY2F_Append_ARB(TOKEN_BUFFER decl_tokens, ARB_HANDLE arb, BOOL purple_assumed_size)
{
       ...
       ARB_const_ubnd(arb))   /* Constant upper bound */
   {
      ...
      {
         if ((ARB_ubnd_val(arb) -ARB_lbnd_val(arb)+ 1LL)>=INT_MAX )   
	 TCON2F_translate(decl_tokens, 
   ...
   else
   {
	 ...
	 Append_Token_Special(decl_tokens, ':');   
      ...
      {
         if ((ARB_ubnd_val(arb) + 1LL)>=INT_MAX )
	 ...
	 Append_Token_Special(decl_tokens, ')');
      ...
      {
         if (strncmp(ST_name(ST_ptr(ARB_ubnd_var(arb))),"s$",2)==0) {
           TCON2F_translate(decl_tokens,
                             Host_To_Targ(MTYPE_I4,
                                          1LL),
                             FALSE /*is_logical*/);
         ...
         if (!zero_lbnd)
	 {
	    Append_Token_Special(decl_tokens, ')');
	    Append_Token_Special(decl_tokens, '-');
            if ((ARB_lbnd_val(arb) - 1LL)>= INT_MAX)   
	    TCON2F_translate(decl_tokens, 
			     Host_To_Targ(MTYPE_I8,
					  ARB_lbnd_val(arb) - 1LL),
			     FALSE /*is_logical*/);
...
TY2F_List_Common_Flds(TOKEN_BUFFER tokens, FLD_HANDLE fldlist)
{
  ...
  bool needComma=false; // problem is we need to jump over fields flagged "equivalenced"
  do
    {
      FLD_HANDLE fld (fld_iter);
	...
	FLD_HANDLE next (++next_iter);
        if (!FLD_equivalence(next) && needComma) { 
	  Append_Token_Special(tokens, ',');
	  needComma=false;
	}
      }
