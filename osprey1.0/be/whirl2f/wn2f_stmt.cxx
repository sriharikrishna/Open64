...
#ifdef _KEEP_RCS_ID
/*REFERENCED*/
static char *rcs_id = "$Source: /m_home/m_utkej/Argonne/cvs2svn/cvs/Open64/osprey1.0/be/whirl2f/wn2f_stmt.cxx,v $ $Revision: 1.38.2.4 $";
#endif
...
// This is called from WN2F_Append_Symtab_Vars.
  ...
  void operator() (UINT32 idx , ST* st) const 
    { 
     int testb = !BE_ST_w2fc_referenced(st);
     int testb1 = !ST_has_nested_ref(st);
...
WN2F_do_loop(TOKEN_BUFFER tokens, WN *wn, WN2F_CONTEXT context)
{
   ...
   if (W2F_Emit_Cgtag && loop_info != NULL)
	 ...
	 Concat2_Strings("LOOPINFO #",
			 WHIRL2F_number_as_name((INT64)loop_info)),
	 1,
	 1);
...
//**********************************************
WN2F_STATUS
WN2F_implicit_bnd(TOKEN_BUFFER tokens, WN *wn, WN2F_CONTEXT context)
 {
     Append_Token_Special(tokens, ' ');
  return EMPTY_WN2F_STATUS;
 }
...
WN2F_ar_construct(TOKEN_BUFFER tokens, WN *wn, WN2F_CONTEXT context)
{
  INT kid;
   ...
   for (kid = 0; kid < WN_kid_count(wn); kid++) {
      ...
      (void)WN2F_translate(tokens,WN_kid(wn,kid), context);
