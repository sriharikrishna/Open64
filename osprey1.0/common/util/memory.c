...
)
{
  ...
  if (purify_pools) {
    ...
    if (old_block && !foundit) {
      ...
      memmove((MEM_PTR) (((size_t) ret_val)+8), old_block, old_size);
    }
    else {
  ...
  if (old_size <= MIN_LARGE_BLOCK_SIZE) {
    if (new_size < old_size)
      ...
      memmove (result, old_block, old_size);
      return result;
    }
  } else {
	...
	result = Raw_Allocate (pool, new_size);
	memmove (result, old_block, new_size);
	...
	if (MEM_POOL_bz(pool)) {
	  memset (((char *) MEM_LARGE_BLOCK_ptr(large_block)) + old_size,
		 '\0', new_size - old_size);
	}
 ...
 *            (NOT copied.)
 *  'memzero' - if true, memory allocate from the pool will always be
