...
static OPCODE is_logop(OPCODE opc)
{
   ...
   if (op == OPR_LAND || op == OPR_LIOR) return (opc);
   ...
   for (i=0; i <= 1; i++) {
	 ...
	 if (s[i]==s[j] && (dt = simp_diff_value(x[i],x[j],s[i]))) {
	     ...
	     */
		  ...
		  if (s_lhs) {
		     lhs = SIMPNODE_SimpCreateExp1(OPC_FROM_OPR(OPR_NEG,ty),lhs);
		  }
		  if (s_rhs) {
		     rhs = SIMPNODE_SimpCreateExp1(OPC_FROM_OPR(OPR_NEG,ty),rhs);
		  }
	...
	if (((SIMPNODE_operator(k0k0) == OPR_LSHR) ||
	 ...
	 if (c2 != 0) {
	    if ((c1/c2)*c2 == c1) { 
	       ...
	       r = SIMPNODE_SimpCreateExp2(opc,SIMPNODE_kid0(k0),
					   SIMP_INTCONST(OPCODE_rtype(firstop), c1/c2));
	       SIMP_DELETE(SIMPNODE_kid1(k0));
	       SIMP_DELETE(k0);
	       SIMP_DELETE(k1);
	    } else {
...
simpnode SIMPNODE_SimplifyPstore(OPCODE opc, WN_OFFSET offset,
                                 TY_IDX ty, UINT field_id, simpnode value,
                                 simpnode addr)
{
   simpnode r = NULL;
   ...
   if (SIMPNODE_operator(addr) == OPR_LDA) {
      lda_offset = SIMPNODE_lda_offset(addr);
      if (is_add_ok(&new_offset,offset,lda_offset,MTYPE_I4)) {
         SHOW_RULE("PSTORE(LDA)->PSTID");
         pointed = TY_pointed(ty);
         DevAssert(pointed, ("TY_pointed of PSTORE type is NULL"));
  r = WN_CreatePStid(OPCODE_operator(opc) == OPR_PSTORE ? OPR_PSTID : OPR_STBITS,
                           OPCODE_rtype(opc),
                           OPCODE_desc(opc),
                           new_offset,
                           SIMPNODE_st_idx(addr),
