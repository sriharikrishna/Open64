...
# ifdef _TARGET64
# define CONVERT_INT_CONST(TYPE, LEN, RESULT)                                 \
                { aligned_value_type a_const, tmp_const;                      \
		  int _stat, _bits, _base, _cvrt_lin_type;                    \
                  const_buf[(LEN)] = '\0';                                    \
                 _base = 10;                                                  \
                 _stat = AR_convert_str_to_int((AR_DATA *)tmp_const.v,        \
                 ...
                 if (RESULT == TRUE)  {                                       \
                    ...
                    _stat = AR_convert((AR_DATA *)a_const.v,                  \
                         (const AR_TYPE *)&linear_to_arith[_cvrt_lin_type],   \
                              (const AR_DATA *)tmp_const.v,                   \
                       (const AR_TYPE *)&input_arith_type[TYP_LINEAR(TYPE)]); \
                    SHIFT_ARITH_RESULT(a_const.v, TYP_LINEAR(TYPE));          \
                    if (((_stat & AR_STAT_OVERFLOW) != 0)   ||                \
                        ((_stat & AR_STAT_UNDERFLOW) != 0)  ||                \
                        ((_stat & AR_STAT_SEMIVALID) != 0)  ||                \
                        ((_stat & AR_STAT_UNDEFINED) != 0)) {                 \
...
# define CONVERT_REAL_CONST(TYPE, LEN, RESULT)                                \
                { aligned_value_type a_const;                                 \
                  int _stat;                                                  \
                  const_buf[(LEN)] = '\0';                                    \
                 _stat = AR_convert_str_to_float((AR_DATA *)a_const.v,        \
                         (const AR_TYPE *)&input_arith_type[TYP_LINEAR(TYPE)],\
                               (const char *)const_buf);                      \
                 SHIFT_ARITH_RESULT(a_const.v, TYP_LINEAR(TYPE));             \
                 if (((_stat & AR_STAT_OVERFLOW) != 0)   ||                   \
...
# define CONVERT_INT_CONST(TYPE, LEN, RESULT)				      \
		{ long_type	a_const;                                      \
		  const_buf[(LEN)] = '\0';                                    \
                  errno = 0;                                                  \
		  a_const = strtol(const_buf, (char **) NULL, 10);            \
                  if (errno == 0) {                                           \
		     TOKEN_CONST_TBL_IDX(token) = ntr_const_tbl(TYPE,         \
                                                            FALSE, &a_const); \
                  } else {                                                    \
...
# define CONVERT_REAL_CONST(TYPE, LEN, RESULT)	          		      \
		{ long_type a_const[MAX_WORDS_FOR_NUMERIC];                   \
                  double tmp_const;                                           \
                  int	_stat;                                                \
		  const_buf[(LEN)] = '\0';                                    \
                  errno = 0;                                                  \
		  tmp_const = strtod(const_buf, (char **)NULL);               \
                  if (errno != 0) {                                           \
                    OVERFLOW_MESSAGE(RESULT);                                 \
                  }                                                           \
                  else {                                                      \
                     _stat = AR_convert((AR_DATA *)a_const,                   \
                          (const AR_TYPE *)&linear_to_arith[TYP_LINEAR(TYPE)],\
                               (const AR_DATA *)&tmp_const,                   \
                     (const AR_TYPE *)&linear_to_arith[Real_8]);              \
                     SHIFT_ARITH_RESULT(a_const, TYP_LINEAR(TYPE));           \
                    if (((_stat & AR_STAT_OVERFLOW) != 0)   ||                \
                   /*   ((_stat & AR_STAT_UNDERFLOW) != 0)  ||   */           \
                        ((_stat & AR_STAT_SEMIVALID) != 0)  ||                \
                        ((_stat & AR_STAT_UNDEFINED) != 0)) {                 \
...
# define CONVERT_DBL_CONST(TYPE, LEN, RESULT)			              \
		{ double a_const;                                             \
		  const_buf[(LEN)] = '\0';                                    \
                  errno = 0;                                                  \
		  a_const = strtod(const_buf, (char **)NULL);                 \
                  if (errno == 0) {                                           \
		    TOKEN_CONST_TBL_IDX(token) = ntr_const_tbl(TYPE,          \
...
# define CONVERT_INT_CONST(TYPE, LEN, RESULT)				      \
		{ long_type	a_const;                                      \
		  const_buf[(LEN)] = '\0';                                    \
                  errno = 0;                                                  \
		  a_const = strtoll(const_buf, (char **) NULL, 10);           \
                  if (errno == 0) {                                           \
		     TOKEN_CONST_TBL_IDX(token) = ntr_const_tbl(TYPE,         \
                                                            FALSE, &a_const); \
                  } else {                                                    \
...
# define CONVERT_REAL_CONST(TYPE_IDX, LEN, RESULT)			      \
		{ float_type a_const;                                         \
		  const_buf[(LEN)] = '\0';                                    \
                  errno = 0;                                                  \
		  a_const = (float_type)atof(const_buf);                      \
                  if (errno == 0) {                                           \
...
# define CONVERT_DBL_CONST(TYPE_IDX, LEN, RESULT)			      \
		{ ldouble a_const;                                            \
		  const_buf[(LEN)] = '\0';                                    \
                  errno = 0;                                                  \
		  a_const = (ldouble)atof(const_buf);                         \
                  if (errno == 0) {                                           \
		    TOKEN_CONST_TBL_IDX(token) = ntr_const_tbl(TYPE_IDX,      \
                                                FALSE, (long_type *)&a_const);\
                  } else {                                                    \
