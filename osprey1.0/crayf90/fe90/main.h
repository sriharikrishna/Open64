...
/* 
 * Solaris workaround
 * BUG? Note in tokens.h, Tok_Dir_End-Tok_Dir_Start == 108, but here
 * directive_str only had 98 strings, which means the last 10 strings
 * were NULL pointers! This causes  Segmentation Fault on Solaris in
 * debug.c (function print_cmd_tbl, line997, for loop)! Although it's
 * okay to printf("%s", NULL) on SGI machine.
 * I added the missing strings to the list.
 * 
 */
