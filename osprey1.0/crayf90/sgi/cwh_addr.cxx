...
extern void
fei_subscr_src_triplet(TYPE result_type )
{
  WN *lb  ;
  WN *ub  ;
  WN *str ;
...
if (cwh_stk_get_class() == WN_item) {
  ...
  wt  = cwh_addr_src_triplet(lb,ub,str);
...
cwh_addr_do_bounds_check(WN *subscript_in, WN *lbound, WN *extent)
{
  ...
  if (WNOPR(subscript)==OPR_SRCTRIPLET) {
...
cwh_addr_load_ST(ST * st, OFFSET_64 off, TY_IDX dty)
{
  ...
  case SCLASS_COMMON1:
    ...
    case KIND_POINTER :	    
      fg |= ACCESSED_STORE | ACCESSED_ILOAD | ACCESSED_ISTORE|ACCESSED_PSTORE;
 ...
 * cwh_addr_pstid
 *
 * Create an OPC_PSTID, given an ST, offset, ty
 ...
 */
extern WN *
cwh_addr_pstid(ST *st, OFFSET_64 off, TY_IDX ty , WN * rhs)
{
  WN * wn ;
  WN * wt ;
  TY_IDX tl ;
  ...
  if (BIG_OFFSET(off)) {
    wn = cwh_addr_lda(st,off,ty);
    wn = cwh_addr_pstore(wn,0,ty,rhs);
    ...
    if (IS_ALTENTRY_TEMP(st)) {
      if (MTYPE_is_integral(bt)) {
        ...
        if ((bt == MTYPE_C4) && (ST_ofst(st) != 0)) {
          ...
          op  = Stid_Opcode [bt];
          wn  = WN_CreatePStid (op,off,st,ty,WN_COPY_Tree(rhs));
          cwh_block_append(wn);
          ...
          op  = PStid_Opcode [bt];
          wn  = WN_CreatePStid (op,4,ST_base(st),tl,wt);
          cwh_block_append(wn);
...
# endif
     ...
     op  = PStid_Opcode [bt];
     wn  = WN_CreatePStid (op,off,st,tl,rhs);
  }
 ...
 * cwh_addr_pstore
 *
 * Create an OPC_PSTORE, given an address, offset,
 * scalar ty and a rhs.
 *
 ====================================================
 */
extern WN *
cwh_addr_pstore(WN * lhs, OFFSET_64 off, TY_IDX ty, WN * rhs)
{
  WN * wn ;
  TY_IDX tp ;
  OPCODE op ;
...
cwh_addr_store_ST(ST * st, OFFSET_64 off, TY_IDX dty,  WN * rhs) 
{
  ...
  case SCLASS_COMMON1:
    ...
    case KIND_POINTER:
      fg |= ACCESSED_LOAD | ACCESSED_ILOAD | ACCESSED_ISTORE|ACCESSED_PSTORE;
      
    case KIND_SCALAR :
      if (ST_class(st)==CLASS_VAR && ST_auxst_is_auto_or_cpointer(st)) {
   	wa = cwh_addr_address_ST(st);
   	cwh_addr_store_WN(wa,off,0,rhs);
   	fg |= ACCESSED_ISTORE|ACCESSED_PSTORE;
 ...
 */
extern void
cwh_addr_pstore_ST(ST * st, OFFSET_64 off, TY_IDX dty,  WN * rhs)
{
  WN * wn;
  WN * wa;
  TY_IDX ts;
  ...
  case SCLASS_FORMAL:
    ...
    if (BY_VALUE(ty)) {
      wn = cwh_addr_pstid(st,0,ts,rhs);
      cwh_block_append(wn) ;

    } else {
      wa = cwh_addr_address_ST(st);
      cwh_addr_pstore_WN(wa,off,ts,rhs);
    }
    break ;
  ...
  case SCLASS_COMMON1:
    ...
    case KIND_SCALAR :
      if (ST_class(st)==CLASS_VAR && ST_auxst_is_auto_or_cpointer(st)) {
        wa = cwh_addr_address_ST(st);
        cwh_addr_pstore_WN(wa,off,0,rhs);
        fg |= ACCESSED_ISTORE|ACCESSED_PSTORE;

      } else {
        wn = cwh_addr_pstid(st,off,ts,rhs);
        cwh_block_append(wn) ;
...
# if ! defined (linux)
        if (IS_ALTENTRY_TEMP(st)) {
          if (TY_mtype(ts) == MTYPE_CQ){
            if(!ST_auxst_altentry_shareTY(ST_base(st))) {
              wn = cwh_addr_load_ST(st,0,NULL);
              cwh_addr_pstore_ST(Altaddress_ST,0,NULL,wn);
            }
          }
        }
# endif
    ...
    case KIND_STRUCT:
        ...
        if (TY_kind(ts) == KIND_SCALAR) {
          ...
          wa = cwh_addr_address_ST(st,off);
          cwh_addr_pstore_WN(wa,0,ts,rhs);
        }
      }
      break ;
 ...
 * cwh_addr_pstore_WN
 *
 * Create an OPC_PSTORE, or OPC_MSTORE given an
 * address and WN. Use the TY of the address, unless
 * the the TY argument is not NULL, when it's
 * probably a derived type component.
 *
 ...
 */
extern void
cwh_addr_pstore_WN(WN * lhs, OFFSET_64 off, TY_IDX dty, WN * rhs)
{
  WN * wn ;
  TY_IDX ts ;
  TY_IDX ty ;
  ...
  case KIND_SCALAR:
  case KIND_POINTER:
    wn = cwh_addr_pstore(lhs,off,ty,rhs);
    break ;
  ...
  case KIND_ARRAY:
    ...
    if (TY_kind(ts) == KIND_STRUCT)
      ...
      wn = cwh_addr_pstore(lhs,off,ts,rhs);
    break;
 ...
 *
 * Make an OPC_SRCTRIPLET - we get an upper bound,
 * but need an extent.
 *
 *===============================================
 */
static WN *
cwh_addr_src_triplet(WN *lb,WN *ub,WN *str)
{
  WN * wn ;

  wn = WN_Create (opc_src_triplet, 3) ;
  WN_kid0(wn) = lb;
  WN_kid1(wn) = ub;
  WN_kid2(wn) = str;
...
{
  return(WN_opcode(wn) == opc_triplet || WN_opcode(wn) ==opc_src_triplet);
}
...
cwh_addr_nonc_recast(WN *wt, WN *wa)
{
  ...
  FOREACH_AXIS(i,nk) {
	...
	DevAssert((WNOPR(sc0) == OPR_SRCTRIPLET),("nonc rhs"));
...
printf("here4444\n");
  ...
  DevAssert((WNOPR(WN_kid(wt,1+SUB_OFF(2))) == OPR_SRCTRIPLET),(" No triplet"));
...
cwh_addr_init_target(void)
{
  ...
  if (Pointer_Size == 4) {
    ...
    opc_triplet  = OPC_I4SRCTRIPLET ;
    opc_src_triplet  = OPC_I4SRCTRIPLET ;
    ...
    opc_triplet  = OPC_I8SRCTRIPLET ;
    opc_src_triplet  = OPC_I8SRCTRIPLET ;  
    cwh_addr_char_len_typeid = MTYPE_I4;
