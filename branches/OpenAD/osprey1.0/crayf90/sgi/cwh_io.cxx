...
#ifdef _KEEP_RCS_ID
  static char *rcs_id = " $Id: cwh_io.cxx,v 1.8.2.1 2004-10-11 20:45:28 eraxxon Exp $ ";
#endif /* _KEEP_RCS_ID */
...
fei_IO_list (int num_args, int mode )
{
  ...
  for (i=0; i < num_args; i++) {
    ...
    if (!NAMELIST_MODE(mode)) {
        cray_type_code = cwh_expr_operand(NULL);
...
fei_implied_do(void)
{
 ...
 for (i = 0; i < num_list_items; i++) 
      iolist[i] = cwh_expr_operand(NULL);

 incr = cwh_expr_operand(NULL);

 stop = cwh_expr_operand(NULL);

 start = cwh_expr_operand(NULL);
  ...
  for (item=OPEN_ERR; item >= OPEN_CALLNAME; item--) {
	...
	case OPEN_IOSTAT:
	   ...
	   case ST_item:
               ...
               if (item == OPEN_RECL)
                 wn = WN_CreateIoItem1(IOC_RECL, wn, ts);
               else if (item == OPEN_IOSTAT)
                 wn = WN_CreateIoItem1(IOC_IOSTAT, wn, ts);
               else if (item == OPEN_UNIT)
