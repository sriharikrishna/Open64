 ...
 * $Author: dsystem $
 * $Source: /m_home/m_utkej/Argonne/cvs2svn/cvs/Open64/osprey1.0/libm/mips/cos.c,v $
 *
 * Revision history:
 *  09-Jun-93 - Original Version
 *
 * Description:	source code for cos function
 *
 * ====================================================================
 * ====================================================================
 */

static char *rcs_id = "$Source: /m_home/m_utkej/Argonne/cvs2svn/cvs/Open64/osprey1.0/libm/mips/cos.c,v $ $Revision: 1.1.1.1 $";
 ...
 *	  be kept in sync with this routine and sin() so that it
...
struct exception	exstruct;
	...
	if ( xpt < 0x7fd )
	{
		...
		if ( xpt >= 0x7c2 )
		{
			...
			poly = (((((Q[6].d*xsq + Q[5].d)*xsq + Q[4].d)*xsq +
				Q[3].d)*xsq + Q[2].d)*xsq + Q[1].d)*xsq + one.d;
	...
	*/
	...
	if ( (x != x) || (fabs(x) == Inf.d) )
	{
		...
		{
			fprintf(stderr, "domain error in cos\n");
			SETERRNO(EDOM);
		}
	...
	/* just give up and return 0.0, setting errno = ERANGE */
		...
		if ( matherr( &exstruct ) == 0 )
		{
			fprintf(stderr, "range error in cos (total loss \
of significance)\n");
			SETERRNO(ERANGE);
		}
