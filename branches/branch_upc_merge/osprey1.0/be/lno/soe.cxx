...
#include <sys/int_limits.h> //for INT32_MIN INT32_MAX
#endif
...
// replace abs() with labs() otherwise compiler will complain
// there are a number of same changes below 
      if (labs(tmp) > INT32_MAX) return(FALSE);  // overflow
...
// Solaris CC workaround
  if (labs(row1c) > INT32_MAX) return(0);  // overflow
  if (labs(row2c) > INT32_MAX) return(0);  // overflow
...
// Solaris CC workaround
	if (labs(tmp) > INT32_MAX) return(FALSE);
...
// Solaris CC workaround
  if (labs(_work_const_eq[eqnum]) % g != 0) {
    *proved_inconsistent = TRUE;
    return(-1);
  }
