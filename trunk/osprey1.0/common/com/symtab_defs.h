...
enum ST_SCLASS
{
    ...
    SCLASS_MODULE       = 18,          // For keep module as a storage class
                                       // instead of generating common block
    SCLASS_COMMON1      = 19,           

    SCLASS_COUNT	=  20		// total number of classes
...
enum FLD_FLAGS
{
    ...
    FLD_IS_POINTER      = 0x0080        // is f90_pointer
};
 ...
 *
 * There are several issues on this bit field 'unused'. First, Solaris CC
 * doesn't allow long long bit-field. So we'll have to change it to long. 
 * Second, Solaris CC has different implementation on bit field, such that
 * if we define "mUINT32 unused : 40", the total size of PU will be  32
 * bytes instead of 24 bytes!  Although it seems to be okay for 'mfef90',
 * it does cause Seg Fault on 'ir_b2a'. One simple way is to define 
 * 'unused' as 32-bit long, and it's up to the compiler to acutally make
 * PU 24 bytes. See symtab_verify.cxx for the use of "unused".. 
 *
 */
