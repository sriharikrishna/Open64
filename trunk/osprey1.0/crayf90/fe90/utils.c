...
/* SGI's gcc -3.0 seems to have a bug? including the following cases cause
 * the switch to fall down to default no matter what the value of seg_len
 * is.
 */
/* #if 0 */ /* must if zero this and below! why? */
      case 23:
         while (seg_len > 32) {
            for (idx = first + stride -1 ; idx <= last; idx += stride) {
               tst_val = DIFF(1);
