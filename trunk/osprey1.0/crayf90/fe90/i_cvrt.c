...
# ifdef _ENABLE_FEI
  if (ATP_PGM_UNIT(pgm_attr_idx)==Module)  
      ismodule=1;
   if (ATP_PGM_UNIT(pgm_attr_idx)==Blockdata)
      ismodule=2;
   PDGCS_new_proc(2000,
...
# endif
        ...
        PDG_DBG_PRINT_START
        PDG_DBG_PRINT_C("fei_pstore");
        PDG_DBG_PRINT_END

# ifdef _ENABLE_FEI
        fei_pstore(pdg_type_void);
# endif
        break;
...
/*        fei_subscr_triplet(pdg_type_void); */
        fei_subscr_src_triplet(pdg_type_void);
# endif
        break;
...
# if defined(COARRAY_FORTRAN)
/* co_dim could be 0 if the coarray referenced without co_rank */
/*        co_dim = BD_RANK(pe_bd_idx);  */

       co_dim = IR_LIST_CNT_R(ir_idx)-BD_RANK(bound_idx);
...
# if defined(COARRAY_FORTRAN)  && defined(_TARGET_OS_MAX) 
...
# endif
               ...
               if (BD_ARRAY_CLASS(pe_bd_idx)!= Deferred_Shape &&  
                   BD_ARRAY_CLASS(pe_bd_idx)!= Deferred_Shape1)

                 cvrt_exp_to_pdg(BD_LB_IDX(pe_bd_idx, co_dim),
                                 BD_LB_FLD(pe_bd_idx, co_dim));
...
# endif
               ...
               if (BD_ARRAY_CLASS(pe_bd_idx)!= Deferred_Shape &&  
                   BD_ARRAY_CLASS(pe_bd_idx)!= Deferred_Shape1)
                 cvrt_exp_to_pdg(BD_XT_IDX(pe_bd_idx, co_dim),
                                 BD_XT_FLD(pe_bd_idx, co_dim));
                 else fei_implicit_expr();
              }
...
# if defined(_EXTENDED_CRI_CHAR_POINTER)
                       ...
                       TYP_CHAR_CLASS(ATD_TYPE_IDX(attr_idx)) == 
                                                           Assumed_Size_Char) {
                 cvrt_exp_to_pdg(BD_SM_IDX(pe_bd_idx, co_dim),
                                 BD_SM_FLD(pe_bd_idx, co_dim));
...
# endif
              ...
              if (BD_ARRAY_CLASS(pe_bd_idx) == Assumed_Size) {
                 if (BD_RANK(pe_bd_idx) == IR_LIST_CNT_R(ir_idx) && ss == 0) {
                    bound_chk = FALSE;
                 } 
              } 
...
#if 0
/*  here add send all rename as ST item
    here add fei_use(module_name,boolen with only or not)
*/
           if (ATP_USE_LIST(IR_IDX_L(ir_idx) != NULL_IDX))
                printf("rename_only_list?\n");
#endif
...
static TYPE get_type_desc(int	input_idx)
{
   ...
   long64		ubound;
   int      		flag 		= 0;
   boolean		lb_vble         = FALSE;
   boolean		lb_symcon	= FALSE;
   boolean              ub_vble         = FALSE;
   boolean              ub_symcon       = FALSE;
   boolean		xt_vble         = FALSE;
   boolean		xt_symcon	= FALSE;
   boolean              lb_empty        = FALSE;
   boolean              ub_empty	= FALSE;
   boolean              xt_empty        = FALSE;
/*   boolean              ub_star         = FALSE;   */
   long			pdg_array_idx;
   int			rank;
...
# endif
         ...
         if (BD_UB_FLD(bd_idx, i) == NO_Tbl_Idx) {
...
# endif
         ...
         if (BD_XT_FLD(bd_idx,i)  == NO_Tbl_Idx) {
             xt_empty = TRUE;
             span = (long64) 0;
         }
	 else {
...
# endif
         ...
         flag = (lb_symcon << FEI_ARRAY_DIMEN_SCON_LB)|
                ...
                (lb_empty  << FEI_ARRAY_DIMEN_EMPTY_LB)|
...
# if defined(GENERATE_WHIRL)
         if (i == BD_RANK(bd_idx) && pe_bd_idx == 0) {   
# endif
            PDG_DBG_PRINT_START
            PDG_DBG_PRINT_C("fei_descriptor");
...
# endif
         ...
         if (BD_UB_FLD(pe_bd_idx, i) == NO_Tbl_Idx) {
...
# endif
         ...
         if (BD_XT_FLD(pe_bd_idx,i)  == NO_Tbl_Idx) {
             xt_empty = TRUE;
             span = (long64) 0;
         }
         else {
...
# endif
      ...
      PDG_DBG_PRINT_START
      PDG_DBG_PRINT_C("fei_co_array_dimen");
      PDG_DBG_PRINT_O("(1) flags", flag);
      PDG_DBG_PRINT_LLD("(2) lbound", lbound);
      PDG_DBG_PRINT_LLD("(3) extent", extent);
      PDG_DBG_PRINT_D("(4) rank", i);
...
# endif
  ...
  if (SB_MODULE_IDX (sb_idx)!=NULL_IDX)
   ...
   if (SB_SCP_IDX(sb_idx) != curr_scp_idx) {
      ...
      else if (sb_type == Based ||sb_type ==Static) { 
...
# endif
      ...
      if (ATP_PROC(attr_idx) == Intern_Proc) { 
         pdg_alt_entry_idx = parent_idx;
          ...
          {
           if (ATP_INTRIN_ENUM(attr_idx)==Sync_Images_Intrinsic ||
               ATP_INTRIN_ENUM(attr_idx)==Rem_Images_Intrinsic  ||
               ATP_INTRIN_ENUM(attr_idx)==Log2_Images_Intrinsic ||
               ATP_INTRIN_ENUM(attr_idx)==This_Image_Intrinsic  ||
               ATP_INTRIN_ENUM(attr_idx)==Num_Images_Intrinsic)
...
# endif
   ...
   if (cmd_line_flags.debug_lvl <= Debug_Lvl_2) {
      if (ATP_PROC(attr_idx) == Intrin_Proc && FALSE) {  
...

{
   ...
   switch (ATL_CLASS(attr_idx)) {
      ...
      case Lbl_Format:
         fmt_ptr = (char *) &CN_CONST(ATL_FORMAT_TMP(attr_idx));
         break;
...
/* if fmt_ptr is NULL, Irix's and Linux's fprintf will just print (null), but
 * solaris will dump core! Change them all for compatibility.
 */
   PDG_DBG_PRINT_S("(4) fmt_ptr", (fmt_ptr == NULL) ? "null" : fmt_ptr); 
...
|*									      *|
\******************************************************************************/
static void  send_namelist_group(int	ng_attr_idx,int in_model)
...
# endif
      ...
      if (ATD_ARRAY_IDX(attr_idx) != NULL_IDX) {
         ...
         case Deferred_Shape1:
            flag = flag | ((long64) 1 << FEI_OBJECT_DEFERRED_SHAPE);
            break;   
...
# endif
      ...
      if (AT_MODULE_IDX(attr_idx)!=NULL_IDX ||
          AT_ORIG_MODULE_IDX(attr_idx)!=NULL_IDX) {
        flag = flag | ((long64) 1 << FEI_OBJECT_EXTERNAL);
      }
...
/*         send_namelist_group(attr_idx,0); add a argument for external??? */
       ...
       if (AT_MODULE_IDX(attr_idx)!=NULL_IDX ||
         ...
         send_namelist_group(attr_idx,0);
      }
     else {
       if (AT_MODULE_IDX(attr_idx)!=NULL_IDX ||
          AT_ORIG_MODULE_IDX(attr_idx)!=NULL_IDX) {  /*  input from a module */
       ...
       send_namelist_group(attr_idx,1);
       }
...
#  if defined(_TARGET_OS_MAX) || defined(_TARGET_OS_UNICOS)
   ...
   if (ATP_PGM_UNIT(AT_WORK_IDX) != Module ||TRUE) {  /* Inlinable situation */ 
