...
/* cf90.cat needs to be installed in /usr/lib/locale/C/LC_MESSAGES,
 * another way is just to give it an explicit pathname - we can define
 * a macro in Makefile for this purpose.
 */
   msg_sys = catopen(CF90CATPATHNAME, 0); 
...
# if !defined(_HOST_OS_LINUX)
      ...
      if (__catgetmsg_error_code() == NL_CAT_LOCALE) {
         fprintf(stderr, "cft90 INTERNAL: "
...
/* this catopen also has to be changed, otherwise the final print
 * of error messages will be uncorrect (either no detailed message
 * or print some nonsense). SGI is okay.
 */
   message_catalog = catopen(CF90CATPATHNAME, 0);
